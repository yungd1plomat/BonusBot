...
public class InitEditBonusHandler : IHandler
{
    public ITelegramBotClient Client { get; set; }

    public InitEditBonusHandler(ITelegramBotClient client)
    {
        Client = client;
    }
    ...
    {
        return message.Text == "Редактировать бонус" && user.IsAdmin;
    }

    public async Task Handle(Message message, TGUser user, AppDb db)
    {
        StringBuilder sb = new StringBuilder();
        sb.AppendLine("Выберите ID бонуса для редактирования:");
        sb.AppendLine();
