...
public class InitRemoveBmHandler : IHandler
{
    public ITelegramBotClient Client { get; set; }

    public InitRemoveBmHandler(ITelegramBotClient client)
    {
        Client = client;
    }
    ...
    {
        return message.Text == "Удалить БК";
    }

    public async Task Handle(Message message, TGUser user, AppDb db)
    {
        StringBuilder sb = new StringBuilder();
        sb.AppendLine("Выберите ID БК для удаления:");
        sb.AppendLine();
